module.exports = {
    ...
    execute(msg, args) {
        msg.channel.send((args.length > 0) ? `${args}, ${complimenti[Math.floor(Math.random() * complimenti.length)]}` : `${ragazza[Math.floor(Math.random() * ragazza.length)]}? La ragazza che sta leggendo questo post.`);
    }
}
...
const ragazza = [
    'Sapevate chi è davvero speciale, divertente, succosa e con le dita dei piedi più gustose',
    'Non dovrei dirlo, però nel mondo qualcuno ha preso una multa per eccesso di bellezza e pompino in corsia di sorpasso. Sapevate chi',
    'Porco Dio'
]